...
#warning "tray!"
	...
	void FooMainWindow::loadPlaylist()
	{
		...
		{
			playlistManager->loadPlaylistFromPls(playlistPath);
		}
	}
	...
	void FooMainWindow::savePlaylist()
	{
		...
		QStringList formats;
		formats.append(FooPlaylistStorage::instance().getFormatNames());
		...
		if(dialog.exec() == QDialog::Accepted)
		{
			...
			QStringList selectedFileToSave = dialog.selectedFiles();

			qDebug() << "Wybrany filtr" << sFilter;
			qDebug() << "Wybrany plik" << selectedFileToSave;

			if (!selectedFileToSave.isEmpty())
			{
				if (sFilter == "PLS Playlist (*.pls)")
				{
					playlistManager->savePlaylistToPls(selectedFileToSave.at(0));
				}
				else
				{
					playlistManager->savePlaylistToM3u(selectedFileToSave.at(0));
				}
			}
		}
	}
